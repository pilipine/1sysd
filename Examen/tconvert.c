...
int main(void) {
	...
	if (scanf("%d", &choix) != 1) {
    	...
    	if (choix != 1 && choix != 2) {
        	fprintf(stderr, "Erreur : choix doit etre 1 ou 2.\n");
        	return EXIT_FAILURE;
    	}

    	printf("Entrz la valeur a convertir (cela peut etre aussi de décimal comme par  exemple (36.6) : ");
	if (scanft("%lf" , &valeur) != 1) {
	fprintf(stderr, "Erreur : valeur numerique invalide.\n");
        return EXIT_FAILURE;
	}
